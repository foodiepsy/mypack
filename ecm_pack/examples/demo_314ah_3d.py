# demo_314ah_3d.py  ——  314Ah 大电芯 + 多维热模型(1D/2D/3D)演示
#
# 演示：
#   1) 使用内置 314Ah 储能大电芯默认参数（用户指定的精确参数）；
#   2) 同一电芯分别用 1D / 2D / 3D 热模型求解，对比温度场差异；
#   3) 展示三维温度场内部分布切片。
import sys, os
import numpy as np
import matplotlib
...
def main():
    print("===== 314Ah 大电芯 + 多维热模型(1D/2D/3D) =====")
    ...
    print(f"导热: kx={spec.k[0]} ky={spec.k[1]} kz={spec.k[2]} W/(m·K) (各向异性)")
    ...
    for label, dim, nx, ny, nz in configs:
        ...
        print(f"  末态: T_max={stats['T_max [K]']:.3f} T_avg={stats['T_avg [K]']:.3f} "
    ...
    axs[1].set_title(f"3D温度场切片 (Y={th3d.Ly*1e3/2:.1f}mm)")
