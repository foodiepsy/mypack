...
# 覆盖：1) ECM 模型定制  2) 自定义串并联拓扑  3) 热模型集成  4) 自定义电芯间导热
import sys, os
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
# ----------------------------------------------------------------------------
# 0) 定义一个「工业风格」的可定制 ECM 电芯规格
#    OCV 用曲线；R0/R1 随 (温度, 电流, SoC) 变化；含熵热 dU/dT；1 阶 RC。
# ----------------------------------------------------------------------------
def make_ocv():
...
def demo_thermal():
    def run(conduction, label):
        ...
        cells = [ep.ECMCell(sp) for sp in specs]
        netlist, _, _ = ep.setup_circuit(1, 2)  # 1 串 2 并
        ...
        print(f"\n[{label}] 末态温度 K: {np.round(out['Cell temperature [K]'][-1], 3)}")
        return out

    out_adiabatic = run(None, "热-绝热(仅对流)")
    out_coupled = run([(0, 1, 30.0)], "热-自定义导热(0-1 强耦合 G=30)")
...
def plot_all(out_2s2p, out_adiabatic, out_coupled):
    ...
    axs[0, 1].set_title("各电芯 SoC（不均衡放电）"); axs[0, 1].set_xlabel("t [min]"); axs[0, 1].set_ylabel("SoC"); axs[0, 1].legend()
    ...
    axs[1, 0].set_title("温度：绝热 vs 自定义导热（不同产热的电芯被拉平）")
    ...
    fig.tight_layout()
    fig.savefig("/workspace/ecm_pack_demo.png", dpi=130)
    print("\n图表已保存 -> /workspace/ecm_pack_demo.png")
