...
from ecm_pack.thermal3d import Cell3DThermal, CellThermalModel
...
def test_single_node_matches_analytic():
    ...
    # 角点节点(0,0,0)在 x/y/z 三方向均处边界, 控制体 = 全体积 ×½×½×½ = 1/8。
    # S1 修复后 _V_cell 为每节点数组, 须取该节点自身体积。
    # 其暴露的 x0/y0/z0 三面, 因切向控制体亦折半, 每面面积 = 单格面 ×½×½ = 1/4,
    # 故 hA = h·(Ax+Ay+Az)/4（h=8）。首步从均匀初温出发导热项为零, 退化为孤立节点解析解。
    rho_cp_V = 2300.0 * 1000.0 * th._V_cell[0]
    hA = 8.0 * (Ax + Ay + Az) / 4.0
